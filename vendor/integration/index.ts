  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('mars');

        const virtualModuleId = 'mars:config';
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`MARS \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`MARS config has been loaded.`);
        }
      },
